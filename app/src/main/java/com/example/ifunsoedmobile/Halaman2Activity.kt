...
class   Halaman2Activity : AppCompatActivity() {
    ...
    private fun initListener() {
        ...
        binding.layoutIg.root.setOnClickListener {
            ...
            val intent = Intent(Intent.ACTION_VIEW, instagramUri) // Corrected line
            startActivity(intent)
        }

        binding.layoutEmail.root.setOnClickListener {
            val intent = Intent(Intent.ACTION_SENDTO).apply {
                data = "mailto:${getString(R.string.email)}".toUri() // Potential issue here
            }
            startActivity(intent)
        }
        ...
        binding.layoutPhone.root.setOnClickListener {
            val intent = Intent(Intent.ACTION_DIAL).apply {
                data = "tel:${getString(R.string.telepon)}".toUri() // Potential issue here
            }
            startActivity(intent)
        }
